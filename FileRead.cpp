...
void FileRead(const string& filename)
{
    ...
    while (offset < rar_data.size())
    {
        ...
        if (volume_header->header_type == 0x74)
        {
            ...
            size_t nameOffset = offset + sizeof(VolumeHeaderFormat) + sizeof(FileHead);

            if (nameOffset + file_head->NameSize > rar_data.size())
            {
                break;
            }

            string file_name(reinterpret_cast<const char*>(&rar_data[nameOffset]), file_head->NameSize);

            cout << "Имя файла: " << file_name << endl;
